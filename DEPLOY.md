...
**This package comes configured to connect to a Khulnasoft database over a secure (`wss:`) WebSocket. If you're using a Khulnasoft database, you can ignore what follows.**
...
First, you'll need to set up the proxy itself somewhere public-facing (or on `localhost` for development). See https://github.com/khulnasoft/wsproxy for the Go code and instructions.
...
2. Use experimental pure-JS Postgres connection encryption via [subtls](https://github.com/jawj/subtls). There's no need for nginx in this scenario, and the Postgres connection is encrypted end-to-end. You get this form of encryption if you set both `khulnasoftConfig.useSecureWebSocket`  and `khulnasoftConfig.forceDisablePgSSL` to `false`, and append `?sslmode=verify-full` (or similar) to your connection string. TLS version 1.3 must be supported by the Postgres back-end. **Please note that subtls is experimental software and this configuration is not recommended for production use.**
...
sudo su wsproxy
cd
git clone https://github.com/khulnasoft/wsproxy.git
cd wsproxy
go build
exit
