...
import { khulnasoft, KhulnasoftDbError } from './httpQuery';
import type { KhulnasoftConfigGlobalAndClient } from './khulnasoftConfig';
 ...
 * startup, but currently this works only with Khulnasoft hosts (not vanilla pg or
 * pgbouncer).
 *
...
declare interface KhulnasoftClient {
...
class KhulnasoftClient extends Client {

  get khulnasoftConfig(): KhulnasoftConfigGlobalAndClient { return this.connection.stream; }
  ...
  connect(callback?: (err?: Error) => void) {
    const { khulnasoftConfig } = this;

    // disable TLS if requested
    if (khulnasoftConfig.forceDisablePgSSL) {
      this.ssl = this.connection.ssl = false;
    }

    // warn on double-encryption
    if (this.ssl && khulnasoftConfig.useSecureWebSocket) {
    ...
    const pipelineTLS = khulnasoftConfig.pipelineTLS && this.ssl;
    const pipelineConnect = khulnasoftConfig.pipelineConnect === 'password';

    if (!pipelineTLS && !khulnasoftConfig.pipelineConnect) return result;
...
// copied from pg to support KhulnasoftPool.query
...
class KhulnasoftPool extends Pool {
  ...
  query(config?: any, values?: any, cb?: any) {
    ...
    try {
      ...
      const sql = khulnasoft(connectionString, { fullResults: true, arrayMode: config.rowMode === 'array' });
...
export {
  Socket as khulnasoftConfig,
  KhulnasoftPool as Pool,
  KhulnasoftClient as Client,
  khulnasoft,
  KhulnasoftDbError
};
