export interface KhulnasoftConfigGlobalOnly {
  /**
...
export interface KhulnasoftConfigGlobalAndClient {
  /**
  ...
  */
   ...
   * compatible only with Khulnasoft hosts.
   * 
   * Default: `false`.
   */
  pipelineTLS: boolean;
   ...
   * 
   * On Khulnasoft, disabling SNI and including the Khulnasoft project name in the password
   * avoids CPU-intensive SCRAM authentication, but this is only relevant for
   * earlier iterations of Khulnasoft's WebSocket support.
   * 
   * Default: `false`.
   */
  disableSNI: boolean;
...
export interface KhulnasoftConfig extends KhulnasoftConfigGlobalOnly, KhulnasoftConfigGlobalAndClient { }
