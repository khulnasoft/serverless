...
export class Socket extends EventEmitter {
  ...
  wsProxyAddrForHost(host: string, port: number) {
    ...
    if (wsProxy === undefined) {
      throw new Error(`No WebSocket proxy is configured. Please see https://github.com/khulnasoft/serverless/blob/main/CONFIG.md#wsproxy-string--host-string-port-number--string--string`);
    }
    ...
    try {
      ...
      fetch(fetchAddrFull, { headers: { Upgrade: 'websocket' } })
        ...
        .catch(err => {
          ...
          this.emit('error', new Error(`All attempts to open a WebSocket to connect to the database failed. Please refer to https://github.com/khulnasoft/serverless/blob/main/CONFIG.md#websocketconstructor-typeof-websocket--undefined. Details: ${err.message}`));
          this.emit('close');
        });
    }
  }
  ...
  async startTls(host: string) {
    ...
    if (this.subtls === undefined) throw new Error('For Postgres SSL connections, you must set `khulnasoftConfig.subtls` to the subtls library. See https://github.com/khulnasoft/serverless/blob/main/CONFIG.md for more information.');
