import { Client, ClientBase, Pool, khulnasoft } from '../export';
...
export async function runQuery(queryable: ClientBase | Pool | ReturnType<typeof khulnasoft>, query: Query) {
...
export async function httpRunQuery(n: number, dbUrl: string, ctx: ExecutionContext, query: Query) {
  const sql = khulnasoft(dbUrl, { fullResults: true });
